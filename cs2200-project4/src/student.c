...
#include <stdlib.h>
#include <string.h>
#include <getopt.h> // am I allowed to add getopt.h
...
/* timeslice global variable to keep track of the time for each scheduler algorithm, set in main() */
...
static void schedule(unsigned int cpu_id)
{
    ...
    if (is_empty(rq)) {
        ...
        // lock current and add it to the cpu
...
extern void yield(unsigned int cpu_id)
{
    // add process to the i/o queue, happens when a process is done on the CPU and needs to go to I/O
    pthread_mutex_lock(&current_mutex);
    pcb_t* process = current[cpu_id];
    if (process != NULL) {
    ...
    // figure out what to schedule onto the CPU now that the currently running process is in the I/O request queue
    schedule(cpu_id);
}
...
extern void terminate(unsigned int cpu_id)
{
    // set the process's state to complete
    pthread_mutex_lock(&current_mutex);
    pcb_t* process = current[cpu_id];
    if (process != NULL) {
    ...
    // schedule the next process to the empty CPU
    schedule(cpu_id);
}
...
extern void wake_up(pcb_t *process)
{
    // change the process's state, add it to the ready queue
    process->state = PROCESS_READY;
    pthread_mutex_lock(&queue_mutex);
    enqueue(rq, process);
    pthread_mutex_unlock(&queue_mutex);
    if (scheduler_algorithm == PR) {
        ...
        for (unsigned int i = 0; i < cpu_count; i++) {
            ...
            if (current[i] == NULL) {
                cpuIndex = i;
                break;
            } else if (lowestPriority < current[i]->priority) {
                cpuIndex = i;
                lowestPriority = current[i]->priority;
            } 
        }
        if (current[cpuIndex] != NULL && process->priority < lowestPriority) {
            pthread_mutex_unlock(&current_mutex);
            force_preempt(cpuIndex);
        } else {
            pthread_mutex_unlock(&current_mutex);
        }
    } 
...
{
    if (argc == 4 && 'r' == argv[2][1]) {
        timeslice = *argv[3] - 48;
        scheduler_algorithm = RR;
    } else if (argc == 3 && 'p' == argv[2][1]) {
